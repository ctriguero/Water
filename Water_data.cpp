...
int main( int argc, const char* argv[] )
{
	...
	//----------------------------------------------------------------------------------------------------------------------------
	//----------------------------------------------------------------------------------------------------------------------------	
	unsigned int Lx=100 ;		// Number of atoms 60A/rm
	unsigned int Ly=100 ;		// Number of atoms 60A/rm
	unsigned int Lz=100 ;		// Number of atoms 50A/rm
	const double rm=1.0 ;	// Minimum distance in Angstroms
	int molecules= 3800 ; //1500 ;
	#define QO -1.1128
	#define QH  0.5564	
	//----------------------------------------------------------------------------------------------------------------------------
	//----------------------------------------------------------------------------------------------------------------------------		
...
//	}
	...
	ofstream SOLID ;
	SOLID.open ("water.data", ios::out | ios::trunc); // All computed densities inside cylinders
	...
	SOLID << endl ;
	SOLID << 3*molecules << " atoms" << std::endl ;
	SOLID << 2*molecules << " bonds" << endl ;
	SOLID << 1*molecules << " angles" << endl ;
	SOLID << endl ;
	SOLID << "2" << " atom types" << std::endl ;
	...
	SOLID << rm*(0-Lz/2.0) << "\t" << rm*(Lz-Lz/2.0) << "\t" << "zlo zhi" << std::endl ;
	SOLID << std::endl ; 
	SOLID << "Masses" << std::endl ;
	SOLID << std::endl ; 
	SOLID << "1" << "\t" << "15.999" << std::endl ;
	SOLID << "2" << "\t" << "1.0008" << std::endl ;
	SOLID << std::endl ; 
	...
	std::uniform_real_distribution<> dis_cartesian3(-lzp2+1.5, lzp2-1.5) ;
	...
	SOLID << Atoms << "\t" << mol << "\t" << "1" << "\t" << QO << "\t" << "\t" << owx << "\t" << owy << "\t" << owz << std::endl ;
	Atoms++ ;
	SOLID << Atoms << "\t" << mol << "\t" << "2" << "\t" << QH << "\t" << "\t" << hw2x << "\t" << hw2y << "\t" << hw2z << std::endl ;
	Atoms++ ;
	SOLID << Atoms << "\t" << mol << "\t" << "2" << "\t" << QH << "\t" << "\t" << hw3x << "\t" << hw3y << "\t" << hw2z << std::endl ;
	...
	for (int i = 0; i < mol; ++i)
	{
		SOLID << 2*i+1 << "\t" << "1" << "\t" << 3*i+1 << "\t" << 3*i+2 << endl ;
		SOLID << 2*i+2 << "\t" << "1" << "\t" << 3*i+1 << "\t" << 3*i+3 << endl ;
	}
	...
	for (int i = 0; i < mol; ++i)
	{
		SOLID << i+1 << "\t" << "1" << "\t" << 3*i+2 << "\t" << 3*i+1 << "\t" << 3*i+3 << endl ;
	}
