...
int main( int argc, const char* argv[] )
{
	...
	std::cout << BOLDYELLOW << "          '_) (_`         ADD WATER TO THE CONFIG    " << RESET << std::endl ;
	...
	std::cout << BOLDYELLOW << "    It needs the last frame file: lastframe.lammpstrj" << RESET << std::endl ;
	...
	#define QO -1.1128
	#define QH  0.5564
	int PreviousAtoms=17700 ;		// Previous number of total atoms
	int PreviousWater=4500 ;		// Previos number of water molecules
	//----------------------------------------------------------------------------------------------------------------------------
	//----------------------------------------------------------------------------------------------------------------------------		
...
//	}
	...
	ofstream SOLID ;
	SOLID.open ("solid_add.data", ios::out | ios::trunc); // All computed densities inside cylinders
	...
	SOLID << PreviousWater+molecules << " Water molecules and solid membrane with pore atomic data file" << std::endl ;
	SOLID << endl ;
	SOLID << Atoms+3*molecules+3*PreviousWater << " atoms" << std::endl ;
	SOLID << 2*molecules+2*PreviousWater << " bonds" << endl ;
	SOLID << 1*molecules+1*PreviousWater << " angles" << endl ;
	SOLID << endl ;
	...
	CONFIG << Atoms+3*molecules+13500 << std::endl ;
	CONFIG << std::endl ;
	...
	std::cout  << YELLOW << "    -> Adding already equilibrated water" << std::endl ;
	// data to read in the file
	unsigned int atid, atmo ;
	char atel ; 
	double atx, aty, atz, atvx, atvy, atvz ;
	const char carbon = 'C' ;
	const char oxygen = 'O' ;
	const char hydrogen = 'H' ;
	// string where data is read
	std::string line ;


	ifstream InputFile1 ;
	InputFile1.open ("lastframe.lammpstrj", ios::in ) ;
	...
	while (getline(InputFile1, line))
	{
		...
		aa >> atid >> atmo >> atel >> atx >> aty >> atz >> atvx >> atvy >> atvz ;

		if( (atel == oxygen) || (atel == hydrogen) )
		{
			// Save the same information in the new file. and then add more atoms.
			// Atom number, molecule number, atom type, charge, coordinates
			if( atel == oxygen )
			{
				SOLID << atid << "\t" << atmo << "\t" << "2" << "\t" << QO << "\t" << "\t" << atx << "\t" << aty << "\t" << atz << std::endl ;
				CONFIG << "O" << "\t" << atx << "\t" << aty << "\t" << atz << std::endl ;


				++Oldh2o ;
				cout << RED << "       Inserting equilibrated Molecules: " << BOLDRED << Oldh2o << RESET << RED << "/" << PreviousWater << RESET << "\r";
			}
			if( atel == hydrogen )
			{
				SOLID << atid << "\t" << atmo << "\t" << "3" << "\t" << QH << "\t" << "\t" << atx << "\t" << aty << "\t" << atz << std::endl ;
				CONFIG << "H" << "\t" << atx << "\t" << aty << "\t" << atz << std::endl ;
			}
			xw.push_back (atx) ;
			yw.push_back (aty) ;
			zw.push_back (atz) ;
	...
	std::cout  << YELLOW << "    -> Adding extra water molecules" << std::endl ;
	...
	cout << RED << "       Inserting Molecules: " << BOLDRED << mol << RESET << RED << "/" << molecules << RESET << "\r";
	cout.flush();
	...
	for ( int k=0; k<xw.size(); k++)
	{
		...
		if ( distance < 3.50 ) /// 4.0 normally and 2000 molecules  con 3.5 4500
		{ 
			//cout << "Try again" ;
			goto newmol ; 
		}		
	...
	// Atom number, molecule number, atom type, charge, coordinates
	Atoms++ ;
	SOLID << Atoms << "\t" << SolidAtoms+PreviousWater+mol << "\t" << "2" << "\t" << QO << "\t" << "\t" << owx << "\t" << owy << "\t" << owz << std::endl ;
	Atoms++ ;
	SOLID << Atoms << "\t" << SolidAtoms+PreviousWater+mol << "\t" << "3" << "\t" << QH << "\t" << "\t" << hw2x << "\t" << hw2y << "\t" << hw2z << std::endl ;
	Atoms++ ;
	SOLID << Atoms << "\t" << SolidAtoms+PreviousWater+mol << "\t" << "3" << "\t" << QH << "\t" << "\t" << hw3x << "\t" << hw3y << "\t" << hw2z << std::endl ;
	...
	int TotMol=PreviousWater+molecules ;

	for (int i = 0; i < TotMol; ++i)
	{
		SOLID << 2*i+1 << "\t" << "1" << "\t" << SolidAtoms+3*i+1 << "\t" << SolidAtoms+3*i+2 << endl ;
		SOLID << 2*i+2 << "\t" << "1" << "\t" << SolidAtoms+3*i+1 << "\t" << SolidAtoms+3*i+3 << endl ;
	}
	...
	for (int i = 0; i < TotMol; ++i)
	{
		SOLID << i+1 << "\t" << "1" << "\t" << SolidAtoms+3*i+2 << "\t" << SolidAtoms+3*i+1 << "\t" << SolidAtoms+3*i+3 << endl ;
	}
